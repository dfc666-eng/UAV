...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if step == 0:#起飞
            ...
            if reach_pose(0,0,init_alt+2.5,0.3):
                for i in range(30):
                    send_pose(pose)
                    #rospy.loginfo(pos_data.pose.position)
                step=1
            ...
            if not reach_drop_point:#到达投水点打开串口
                ...
                while not drop:
                    ...
                    try:
                        ...
                        if abs(bucket_pos.x)<=0.05 and abs(bucket_pos.y+0.2)<=0.05 and bucket_pos.z!=0:#桶位于，且不是误识别为000
                            drop_cnt=drop_cnt+1
            ...
            send_pose(pose)
            if reach_pose(world_xy[0],world_xy[1],init_alt+5.5,0.2):
                for i in range(50):
                    send_pose(pose)
                    #rospy.loginfo(pos_data.pose)
                step = 13
