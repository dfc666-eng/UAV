...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        print('alt={0} m, yaw={1} rad'.format(pos_data.pose.position.z,cur_yaw))
        time.sleep(0.5)

    init_alt=sum_alt/10
    init_yaw=sum_yaw/10
    #init_yaw=0
    init_yaw=float(input('initial yaw (degree):'))
    init_yaw=math.radians(init_yaw)
    ...
    while not rospy.is_shutdown():
        ...
        if step == 0:#起飞
            ...
            send_pose(pose)
            #rospy.loginfo(pos_data.pose)
            if reach_pose(world_xy[0],world_xy[1],init_alt+4,0.1) and not reach_drop_point:
            ...
            send_pose(pose)
            #rospy.loginfo(pos_data.pose)
            if reach_pose(world_xy[0],world_xy[1],init_alt+4,0.1):
