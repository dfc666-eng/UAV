...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if step == 0:#起飞
            ...
            if reach_pose(0,0,init_alt+4,0.3):
                for i in range(30):
                    send_pose(pose)
                    #rospy.loginfo(pos_data.pose.position)
                step=1
            ...
            print('hold on')
            for i in range(50):
                pose.pose.position.x = temp_pos.pose.position.x
                pose.pose.position.y = temp_pos.pose.position.y
                pose.pose.position.z = init_alt+2.5 #下降到2.5m
                send_pose(pose)
                #rospy.loginfo(pos_data.pose.position)
            i=0
            print('start searching for bucket...')
            while i<=6.2832:#圆心(30,0) 6*2椭圆
                xx=30+1*math.cos(i)
                yy=-3*math.sin(i)
                rot_xy=[xx,yy]
                world_xy=[rot_xy[0]*math.cos(init_yaw)-rot_xy[1]*math.sin(init_yaw), rot_xy[0]*math.sin(init_yaw)+rot_xy[1]*math.cos(init_yaw)]
                ...
                while not drop:
                    ...
                    try:
                        if (rospy.Time.now() - last_req) > rospy.Duration(3.0):#每隔一段时间进行一次校准
                            ...
                            rot_xy=[0.13+bucket_pos.y,bucket_pos.x]#对准桶
                            world_xy=[rot_xy[0]*math.cos(init_yaw)-rot_xy[1]*math.sin(init_yaw), rot_xy[0]*math.sin(init_yaw)+rot_xy[1]*math.cos(init_yaw)]
